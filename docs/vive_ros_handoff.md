...
On `jscore`, build the ROS Humble bridge image and install the service:

```bash
docker build -t nero-vive-ros:humble -f deploy/vive_ros/Dockerfile .
sudo cp deploy/systemd/nero-vive-ros.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now nero-vive-ros
```

The container uses host networking for ROS discovery and mounts the proven POS
...
| `/nero/reference/pose` | `geometry_msgs/PoseStamped` | `map` | Calibrated K1 body ground truth; calibration required |
| `/nero/reference/path` | `nav_msgs/Path` | `map` | Bounded calibrated ground-truth trajectory |
...
Copy the example and replace both identity transforms with measurements:

```bash
cp config/vive_calibration.example.json config/vive_calibration.json
uv run nero-vive-ros \
  --latest-file /run/nero/vive_pose.json \
  --calibration config/vive_calibration.json
```

With `--calibration`, the bridge computes `T_map_base` and publishes it only as
ground truth under `/nero/reference`. The existing Rerun bridge displays it
beside `/nero/slam`; navigation control does not consume it. The bridge publishes
no map-frame body pose when calibration is absent or tracking is invalid.

For the `jscore` container service, enable the measured file without changing
the unit:

```bash
echo NERO_VIVE_CALIBRATION=/app/config/vive_calibration.json | \
  sudo tee /etc/nero/vive_ros.env
sudo systemctl restart nero-vive-ros
```
...
- No measured controller-to-K1 extrinsic is committed; the generated calibration is ignored.
- No measured Lighthouse-to-map alignment is committed.
