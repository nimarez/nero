...
.spatial{display:grid;grid-template-rows:38px minmax(0,1fr)}.spatial iframe{width:100%;height:100%;border:0;background:#0d1011}.float-tag{position:absolute;z-index:7;right:12px;top:49px;padding:6px 8px;background:#080b0de8;border:1px solid #34424b;font:9px "Liberation Mono",monospace;letter-spacing:.13em;text-transform:uppercase;color:var(--cyan);pointer-events:none}
...
<main>
  ...
  <section class="pane spatial"><div class="panehead"><strong>Spatial model</strong><span class="meta">Perspective · dataframe · box overlays</span></div><iframe id="rerun" src="http://10.2.1.130:8080/rerun/" title="Live Rerun spatial viewer" allow="fullscreen; clipboard-read; clipboard-write"></iframe><div class="float-tag">interactive Rerun view</div></section>
</main>
...
async function rerunHealth(){try{const d=await(await fetch('/api/rerun-health',{cache:'no-store'})).json();badge($('#rerunStatus'),d.reachable)}catch(_){badge($('#rerunStatus'),false)}setTimeout(rerunHealth,2000)}
poll();rerunHealth();
</script></body></html>"""
