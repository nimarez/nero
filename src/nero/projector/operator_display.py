...
<main>
  <section class="stack">
    ...
    <div class="pane telemetry"><div class="panehead"><strong>Data frame</strong><span class="meta">Room frame · floor XY · vertical dropped</span></div><div class="data-grid">
      <div class="readout"><span class="label">Robot position</span><span id="floor" class="value cyan">—</span><small>room_floor metres</small></div>
...
async function poll(){try{const d=await(await fetch('/api/state',{cache:'no-store'})).json(),m=d.motion||{},rp=d.navigation?.robot_pose,detections=d.detections||[];badge($('#camStatus'),!d.camera_error&&d.camera_age_ms<500);badge($('#trackStatus'),!!m.valid);$('#floor').textContent=rp?`${fmt(rp.x)}  /  ${fmt(rp.y)}`:'—';$('#vive').textContent=m.position?`${fmt(m.position[0],2)}  ${fmt(m.position[1],2)}  ${fmt(m.position[2],2)}`:'—';$('#controller').textContent=m.controller_id?`${m.controller_id} · Z ignored · ${fmt(m.age_ms,1)} ms`:'waiting for pose';$('#markers').textContent=detections.length?detections.map(v=>`ID${v.id}`).join('  '):'none';$('#latency').textContent=`${fmt(d.camera_age_ms,0)} / ${fmt(m.age_ms,0)} ms`;drawBoxes(detections)}catch(_){badge($('#camStatus'),false);badge($('#trackStatus'),false)}setTimeout(poll,200)}
