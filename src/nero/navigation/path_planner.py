...
def astar(
    ...
    while open_set:
        ...
        # Explore neighbors
        for nx, ny, move_cost in get_neighbors(
            current.x, current.y, grid, allow_diagonal
        ):
            ...
            if inflation_radius > 0 and grid.is_occupied(
                nx_world, ny_world, radius=inflation_radius
            ):
                continue
